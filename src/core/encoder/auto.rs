...
impl Encoder for AutoEncoder {
    ...
    fn display_name(&self) -> &'static str { "Auto" }
    fn description(&self) -> &'static str { "Pipeline resolver: prefer NVENC/x265 when useful, otherwise copy" }
    ...
    fn encode_args(&self, _profile: &EncodeProfile) -> Vec<String> { Vec::new() }
}
