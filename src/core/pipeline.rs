...
use crate::{
    ...
    db::{entities::{merge_runs, sources}, repos::{merge_runs_repo::{GroupUpdate, MergeRunsRepo}, warnings_repo::WarningsRepo}},
};
...
impl Pipeline {
    ...
    pub async fn run(
        ...
        for (idx, path) in files.iter().enumerate() {
            ...
            self.emit(run.id, "scan", 0, 0, 0, Some(path.to_string_lossy().to_string()), percent);
        }
        ...
        for (idx, group) in groups.iter().enumerate() {
            if cancel.is_cancelled() { anyhow::bail!("cancelled"); }
            let out_dir = output_dir_for_group(&output, &input, group.files.first().map(|item| item.path.as_path()), &source.monthly_subdirs).await?;
            ...
            match result {
                Ok((tracker, fallback_used)) => {
                    ...
                    for warning in warnings {
                        WarningsRepo::add(&self.db, group_model.id, warning.category.clone(), 1, Some(warning.message.clone())).await?;
                    }
                    let warning_level = if fallback_used || !warnings.is_empty() { "warn" } else { "clean" };
                    let status = if fallback_used { "downgraded" } else { "ok" };
                    if fallback_used { downgraded_count += 1; } else { ok_count += 1; }
                    ...
                    MergeRunsRepo::update_group(&self.db, group_model.id, GroupUpdate {
                        start_dt,
                        end_dt,
            ...
            self.emit(run.id, "merge", groups.len(), ok_count, failed_count, Some(out.to_string_lossy().to_string()), percent);
        }
        ...
        write_report(&output.join("_transcode_warnings.txt"), &RunReport { run_id: run.id, status: "succeeded", files: total_files, groups: groups.len(), warnings: failed_count }).await?;
    ...
    fn emit(&self, run_id: uuid::Uuid, phase: &str, group_count: usize, ok_count: usize, failed_count: usize, current_file: Option<String>, percent: f64) {
        let _ = self.progress.send(ProgressEvent { run_id, phase: phase.to_string(), group_count, ok_count, failed_count, current_file, percent });
    }
}
...
fn scan_files(input: &Path) -> anyhow::Result<Vec<PathBuf>> {
    ...
    for entry in WalkDir::new(input).follow_links(false) {
        let entry = entry?;
        if entry.file_type().is_file() && entry.metadata()?.len() > 0 { files.push(entry.path().to_path_buf()); }
    }
    files.sort();
    Ok(files)
}
...
async fn copy_non_video(root: &Path, output: &Path, path: &Path) -> anyhow::Result<()> {
    ...
    if tokio::fs::try_exists(&dest).await.unwrap_or(false) { return Ok(()); }
    tokio::fs::copy(path, dest).await?;
    Ok(())
}

async fn output_dir_for_group(base: &Path, root: &Path, first: Option<&Path>, monthly: &str) -> anyhow::Result<PathBuf> {
    let Some(first) = first else { return Ok(base.to_path_buf()); };
    let rel_dir = first.strip_prefix(root).ok().and_then(Path::parent).unwrap_or_else(|| Path::new(""));
    ...
    let dir = if use_monthly {
        parsed.timestamp.map_or_else(|| base.join(rel_dir), |ts| base.join(format!("{:04}{:02}", ts.year(), ts.month())))
    } else { base.join(rel_dir) };
    tokio::fs::create_dir_all(&dir).await?;
    Ok(dir)
}

fn secs_to_ms(secs: f64) -> i64 { (secs * 1000.0).round() as i64 }

fn file_size(path: &Path) -> Option<i64> { std::fs::metadata(path).ok().and_then(|m| i64::try_from(m.len()).ok()) }
fn sum_file_sizes(paths: &[PathBuf]) -> Option<i64> { Some(paths.iter().filter_map(|p| file_size(p)).sum()) }
...
async fn write_failure_log(out: &Path, inputs: &[PathBuf], reason: &str) -> anyhow::Result<()> {
    let mut text = format!("merge failed: {}\nreason: {reason}\nmembers: {}\n", out.display(), inputs.len());
    for input in inputs { text.push_str(&format!("  {}\n", input.display())); }
    ...
    match source.encoder.as_str() {
        "nvenc-h265" | "x265-veryslow" => {
            ...
            let Some(encoder) = registry.get(&source.encoder) else {
                return runner.concat_copy(inputs, out, cancel).await.map(|tracker| (tracker, true));
            };
            let profile = EncodeProfile::default();
            match runner.concat_encode(inputs, out, &encoder.encode_args(&profile), cancel.clone()).await {
        ...
        _ => runner.concat_copy(inputs, out, cancel).await.map(|tracker| (tracker, false)),
    }
}
