...
impl Orchestrator {
    pub fn new(db: DatabaseConnection, ffmpeg_paths: Arc<tokio::sync::RwLock<FfmpegPaths>>, workers: usize) -> Self {
    ...
    pub async fn enqueue_run(&self, source_id: Uuid, user_id: Uuid) -> anyhow::Result<Uuid> {
        ...
        tokio::spawn(async move {
            ...
            let pipeline = Pipeline::new(this.db.clone(), Arc::clone(&this.ffmpeg_paths), this.progress.clone());
            ...
            if let Err(error) = result {
                ...
                let _ = this.progress.send(ProgressEvent {
                    ...
                    current_file: Some(error.to_string()),
                    percent: 100.0,
                });
            }
